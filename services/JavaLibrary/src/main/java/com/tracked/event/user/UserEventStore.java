...
public class UserEventStore {
    ...
    private final Map<Integer, UserEvent> userDatabase = new HashMap<>();

    @KafkaListener(topics = TrackedKafkaTopic.USER_TOPIC, groupId = "user-group")
    public void listen(ConsumerRecord<Integer, UserEvent> record, Acknowledgment ack) {
        logger.info("Received user event: {}", record.value());
        this.userDatabase.put(record.key(), record.value());

        // Do not ack!
        // We don't want the consumer's offset to update because we always want to replay events
        // from the beginning.
    }
